...
for keys, values in newDict.items():
    if(len(values[0])==5):
        #this if is for the converting list of the subject to the dataframe (having external paper only)
        ...
        #this else if is used to convert the internal subjects marks 
        df = pd.DataFrame(newDict[keys], columns=[keys+'Int', keys+'Tot', keys+'Grd', keys+'Cred'])
        df[keys+'Tot'] = df[keys+'Tot'].replace('AB', '-1')
        df[keys+'Tot'] = df[keys+'Tot'].astype('int')
        newDict[keys]=df
    else:
        #this else is used to convert the name, semester marks, etc. convert to the data frame 
